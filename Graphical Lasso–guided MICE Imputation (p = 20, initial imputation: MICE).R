...
for (run in 1:10) {
  ...
  # 2-2) Initial imputation (single iteration to break NA blocks)
  start_time <- Sys.time()
  ...
  # 2-3) Iterative imputation guided by glasso (BIC-selected rho)
  start_time <- Sys.time()

  for (data_set_number in 1:5) {
    for (iter in 1:5) {
      # Glasso path for a grid of rho
  ...
  # Helper: ROC (FPR, TPR) and AUC given a stack of binary Omegas across rho
  ...
  # Majority vote of edges across multiple datasets at each rho
  ...
  # (iv) MICE with lasso.select.norm (m = 5), then majority vote
  start_time <- Sys.time()
...
# Average ROC across 10 runs per method
